...
#     minThreshold = 50 # Accurate minThresold = 67
#     while True:
...
def recognize_attendence():
    ...
    df = pd.read_csv("EmployeeDetails"+os.sep+"EmployeeDetails.csv")
    font = cv2.FONT_HERSHEY_SIMPLEX
    col_names = ['Id', 'Name', 'Time','Accuracy']
    attendance = pd.DataFrame(columns=col_names)
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if (100-conf) > minThreshold:
                ts = time.time()
                timeStamp = datetime.datetime.fromtimestamp(ts).strftime('%H:%M:%S')
                attendance.loc[len(attendance)] = [Id, aa, timeStamp,confstr]

            tt = tt + " [Pass]" if (100-conf) > minThreshold else tt
...
#   writer =pd.ExcelWriter(temp,engine='openpyxl')
    # attendance.to_excel(writer,sheet_name='a1')
    # attendance.to_excel(writer,sheet_name='a3')
    # reader = pd.read_excel(temp)
    # attendance.to_excel()
    # pd.concat(fileName,attendance,)
    # writer.close()
    # fileName = "Attendance"+os.sep+"Attendance_"+date+"_"+Hour+"-"+Minute+"-"+Second+".csv"
    # fileName ="Attendance"+os.sep+"Attendance_"+date+".csv"
    # temp = os.getcwd()+os.sep+fileName